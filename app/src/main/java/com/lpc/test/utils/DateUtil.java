...
public class DateUtil {
    ...
    public static SimpleDateFormat NOW_TIME3 = new SimpleDateFormat("yyyyMMddHHmmss");
    public static SimpleDateFormat NOW_TIME4 = new SimpleDateFormat("MM-dd HH:mm");
    ...
    public static SimpleDateFormat WEEK_TIME2 = new SimpleDateFormat("EEE HH:mm");
     ...
     * 获得Date的字符串显示
     *
     * @param :NOW_TIME,NOW_DATE,STR_TIME,WEEK_TIME in DateUtil
     * @param date                                  Date
     * @return
     */
    ...
    public static String getDateStrForShow(String dateStr, SimpleDateFormat format) {
        Date date = stringToDate(dateStr, format);
        return getDateStrForShow(date, dateStr);
    }

    public static String getAutoDateStr(String timeStr,
                                        SimpleDateFormat format) {
        String dateStr = timeToDateString(timeStr, DateUtil.NOW_TIME);
        long time = Long.parseLong(timeStr);
        Date date = new Date();
        date.setTime(time);
        return getDateStrForShow(date, dateStr);
     ...
     * date对象与当前时间的对比
     * <p>
     * 当天消息显示时分，之前消息 显示 月日时分
     *
     * @param
     * @param
     * @return
     */
    public static String getDateStrForShow(Date date, String def) {
        if (date == null) {
            return def;
        }
        long nowTime = System.currentTimeMillis();
        String format = DateUtil.NOW_DATE.format(nowTime);
        try {
            Date today = DateUtil.NOW_DATE.parse(format);
...
//		String q = StringUtils.getResString(R.string.before_message);
        // if (ax > 0) {
        // q = "前";
        // }
        // else {
...
//		long x = Math.abs(ax);
//		if (x <= 60 * 1000) {
//			return StringUtils.getResString(R.string.just_message);
//		}
//		if (x <= 60 * 60 * 1000) {
//			return (x / (60 * 1000)) + StringUtils.getResString(R.string.minutes_message) + q;
//		}
//
        if (ax <= 0) {
            return getDateString(date, DateUtil.TIME2);
        }
        ...
        // }
        return getDateString(date, DateUtil.NOW_TIME4);

    }

    public static String getDateStrForShow(long time, String def) {

        return getDateStrForShow(new Date(time), def);
    }
...
//--------------------start----------------------------------
    ...
    public static String fromDeadline(long time) {
        ...
        if (time < ONE_MINUTE) {
            ...
            return "通话" + time / ONE_MINUTE + "分钟";
        } else if (time <= ONE_DAY) {
            return "通话" + time / ONE_HOUR + "小时"
                    + (time % ONE_HOUR / ONE_MINUTE) + "分钟";
        } else {
            long day = time / ONE_DAY;
            long hour = time % ONE_DAY / ONE_HOUR;
            long minute = time % ONE_DAY % ONE_HOUR / ONE_MINUTE;
            return "通话" + day + "天" + hour + "小时" + minute + "分钟";
        }
